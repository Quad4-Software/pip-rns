...
def test_delta_apply_roundtrip(tmp_path):
    from opip.delta import apply_delta, create_delta
    from opip.integrity import build_integrity, collect_files, dump_integrity, file_hash
    from opip.manifest import dump_manifest, make_manifest

    def make_bundle(name, wheels_meta, wheel_bytes_map):
        work = tmp_path / f"work-{name}"
        wdir = work / "wheels"
        wdir.mkdir(parents=True)
        entries = []
        for filename, content in wheel_bytes_map.items():
            path = wdir / filename
            path.write_bytes(content)
            meta = wheels_meta[filename]
            meta = dict(meta)
            meta["sha256"] = file_hash(str(path))
            meta["filename"] = filename
            entries.append(meta)
        manifest = make_manifest(
            name,
            [f"{e['package']}=={e['version']}" for e in entries],
            entries,
            "3.12",
            "any",
        )
        ...
        (work / "requirements.txt").write_text(
            "\n".join(f"{e['package']}=={e['version']}" for e in entries) + "\n",
            encoding="utf-8",
        )
        integrity = build_integrity(collect_files(str(work)), base_dir=str(work))
    ...
    new = make_bundle(
        ...
        {
            "a-1.0-py3-none-any.whl": b"AAAA",
            "b-2.0-py3-none-any.whl": b"BBBB",
        },
    )
    delta_path = tmp_path / "patch.opipd"
    create_delta(str(old), str(new), str(delta_path))
    assert delta_path.is_file()
    applied = tmp_path / "applied.opip"
    apply_delta(str(old), str(delta_path), str(applied))
    assert applied.is_file()
    # Wrong base fails
    import pytest

    from opip.delta import DeltaError

    with pytest.raises(DeltaError):
        apply_delta(str(new), str(delta_path), str(tmp_path / "bad.opip"))
