"""Solid tests for opip delta, extract, offline create, reuse, and CLI."""
...
def test_delta_only_packs_changed_wheels(tmp_path):
    ...
    new_work.mkdir()
    # put both wheels in a staging dir for new bundle
...
def test_delta_apply_roundtrip_verifies(tmp_path):
    whl_a = tmp_path / "a-1.0.0-py3-none-any.whl"
    whl_b = tmp_path / "b-2.0.0-py3-none-any.whl"
    ...
    # New: remove b, keep a, add c, change a content (new build)
    whl_a2 = tmp_path / "a-1.0.0-py3-none-any-v2.whl"
    _make_minimal_wheel(str(whl_a2), name="aaa", version="1.0.0")
    # Force different bytes while keeping filename for change detection
    # by rebuilding a with different file content under same name
    stage_new = tmp_path / "stage-new"
    stage_new.mkdir()
    a_new = stage_new / "a-1.0.0-py3-none-any.whl"
    # append marker so hash differs from old a
    data = whl_a2.read_bytes()
    # rewrite wheel zip with extra file
    import io
    ...
    # Applied a wheel hash matches new bundle
    new_a = next(w for w in manifest["wheels"] if w["filename"].startswith("a-"))
    with zipfile.ZipFile(new_bundle) as zf:
        # compare to new file hash from staging
        assert new_a["sha256"] == file_hash(str(a_new))


def test_apply_rejects_wrong_base(tmp_path):
...
def test_extract_verifies_and_writes_simple_index(tmp_path):
...
def test_extract_fails_on_tampered_bundle(tmp_path):
    whl = tmp_path / "demo-1.0.0-py3-none-any.whl"
    _make_minimal_wheel(str(whl), name="demo", version="1.0.0")
    work = tmp_path / "work"
    work.mkdir()
    ...
    with pytest.raises(ExtractError, match="verification failed"):
        extract_to_wheelhouse(str(bad), str(tmp_path / "out"), verify=True)


def test_create_offline_find_links(tmp_path):
...
def test_create_offline_missing_wheel_fails(tmp_path):
    wheels = tmp_path / "wheels"
    wheels.mkdir()
...
def test_create_reuses_wheels_from_prior_dir(tmp_path):
    wheels = tmp_path / "wheels"
    wheels.mkdir()
    ...
    # Second create should copy from reuse dir, not download
...
def test_cli_delta_apply_json_and_quiet(tmp_path, capsys):
    ...
    # Scarcity: delta smaller than full new bundle when base already held
    assert delta.stat().st_size < new_b.stat().st_size


def test_cli_info_list_json(tmp_path, capsys):
    from opip.cli import main
...
def test_trust_cli_and_verify_uses_default(tmp_path, capsys):
    from opip.cli import main
