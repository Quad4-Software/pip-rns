...
An index is an rngit repository containing a packages.json file:
  {"lxmfy": "926baefe.../quad4/LXMFy", ...}

Indexes are registered by URL, cloned on sync, and merged into a local mapping.
Resolution order: local aliases → synced indexes → raw path.
"""
...
class IndexManager:
    ...
    def _load_packages(self) -> None:
        pkg = self._data_dir / "packages.json"
        if pkg.exists():
            try:
                self._packages = json.loads(pkg.read_text())
            except Exception:
                self._packages = {}
